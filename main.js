...
// 사원정보 API 설정
// IT팀에서 xapikey 발급받으면:
//   1. EMPLOYEE_API_KEY 값을 발급받은 키로 교체
//   2. IS_EMPLOYEE_API_CONNECTED 를 true 로 변경
// ================================================================
const EMPLOYEE_API_BASE = 'https://nbs-api.nmn.io';
const EMPLOYEE_API_KEY  = 'YOUR_XAPI_KEY_HERE';   // ← IT팀 발급 키로 교체
const IS_EMPLOYEE_API_CONNECTED = false;           // ← true 로 변경
...
class VisitorRegistrationForm extends HTMLElement {
                ...
                <div class="section-box">
                    ...
                    <div class="form-group">
                        ...
                        <div class="host-input-row">
                            <input type="text" id="hostDept" name="hostDept" placeholder="부서명 (필수)" required>
    ...
    async _handleSubmit(event) {
        ...
        const hostDept = this.shadowRoot.querySelector('#hostDept').value.trim();
        const hostNameVal = this.shadowRoot.querySelector('#hostName').value.trim();
        const hostInfo = hostDept ? `${hostDept} ${hostNameVal}` : hostNameVal;
        ...
        const data = {
            ...
            hostInfo,
            hostEmpId:     this._selectedHostEmpId || null,
            timestamp:     new Date(),
